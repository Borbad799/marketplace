...
router.get('/stats', (_req, res) => {
  ...
  res.json({
    ...
    realEstate: count("SELECT COUNT(*) AS n FROM listings WHERE type = 'real_estate'"),
    cars: count("SELECT COUNT(*) AS n FROM listings WHERE type = 'cars'"),
    freelance: count("SELECT COUNT(*) AS n FROM listings WHERE type = 'freelance'"),
    clothing: count("SELECT COUNT(*) AS n FROM listings WHERE type = 'clothing'"),
    reports: count("SELECT COUNT(*) AS n FROM reports WHERE status = 'open'"),
    pending: count("SELECT COUNT(*) AS n FROM listings WHERE status = 'pending'"),
    payments: db.prepare('SELECT COALESCE(SUM(amount),0) AS n FROM payments').get().n,
  });
});
...
router.get('/users', (req, res) => {
  ...
  const rows = q
    ? db.prepare('SELECT * FROM users WHERE name LIKE ? OR email LIKE ? OR phone LIKE ? ORDER BY id DESC LIMIT 200').all(q, q, q)
...
router.post('/users/:id/block', (req, res) => {
  ...
  if (next === 'blocked') {
    db.prepare("UPDATE listings SET status = 'archived' WHERE user_id = ? AND status IN ('active','pending','paused')").run(user.id);
  }
...
router.post('/listings/:id/approve', (req, res) => {
  ...
  db.prepare("UPDATE listings SET status = 'active', published_at = datetime('now'), reject_reason = NULL WHERE id = ?").run(listing.id);
  notify(listing.user_id, {
...
router.post('/listings/:id/reject', (req, res) => {
  ...
  db.prepare("UPDATE listings SET status = 'rejected', reject_reason = ? WHERE id = ?").run(reason, listing.id);
  notify(listing.user_id, {
...
router.delete('/listings/:id', (req, res) => {
  db.prepare('DELETE FROM listings WHERE id = ?').run(Number(req.params.id));
  res.json({ ok: true });
});
...
router.post('/reports/:id/resolve', (req, res) => {
  db.prepare("UPDATE reports SET status = ? WHERE id = ?").run(req.body.status || 'resolved', Number(req.params.id));
  res.json({ ok: true });
});
...
router.get('/banners', (_req, res) => {
  res.json({ items: db.prepare('SELECT * FROM banners ORDER BY sort_order, id').all() });
});
...
router.post('/banners', (req, res) => {
  ...
  res.status(201).json({ item: db.prepare('SELECT * FROM banners WHERE id = ?').get(info.lastInsertRowid) });
});
...
router.put('/banners/:id', (req, res) => {
  ...
  db.prepare('UPDATE banners SET title=?, image_url=?, link=?, is_active=?, sort_order=? WHERE id=?').run(
...
router.delete('/banners/:id', (req, res) => {
  db.prepare('DELETE FROM banners WHERE id = ?').run(Number(req.params.id));
  res.json({ ok: true });
});
...
router.post('/cities', (req, res) => {
  ...
  res.status(201).json({ item: db.prepare('SELECT * FROM cities WHERE id = ?').get(info.lastInsertRowid) });
});

router.put('/cities/:id', (req, res) => {
  const c = db.prepare('SELECT * FROM cities WHERE id = ?').get(Number(req.params.id));
  if (!c) return res.status(404).json({ error: 'Не найдено' });
  db.prepare('UPDATE cities SET name=?, name_ru=?, latitude=?, longitude=? WHERE id=?').run(
    req.body.name ?? c.name,
    req.body.nameRu ?? c.name_ru,
    req.body.latitude ?? c.latitude,
    req.body.longitude ?? c.longitude,
    c.id,
  );
  res.json({ ok: true });
});
...
router.delete('/cities/:id', (req, res) => {
  db.prepare('DELETE FROM cities WHERE id = ?').run(Number(req.params.id));
  res.json({ ok: true });
});

router.get('/categories', (_req, res) => {
  res.json({ items: db.prepare('SELECT * FROM categories ORDER BY id').all() });
});
...
router.post('/categories', (req, res) => {
  ...
  res.status(201).json({ item: db.prepare('SELECT * FROM categories WHERE id = ?').get(info.lastInsertRowid) });
});

router.put('/categories/:id', (req, res) => {
  const c = db.prepare('SELECT * FROM categories WHERE id = ?').get(Number(req.params.id));
  if (!c) return res.status(404).json({ error: 'Не найдено' });
  db.prepare('UPDATE categories SET slug=?, name=?, parent_id=?, type=?, icon=? WHERE id=?').run(
    req.body.slug ?? c.slug,
    req.body.name ?? c.name,
    req.body.parentId ?? c.parent_id,
    req.body.type ?? c.type,
    req.body.icon ?? c.icon,
    c.id,
  );
  res.json({ ok: true });
});
