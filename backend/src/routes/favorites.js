...
router.post('/:listingId', requireAuth, async (req, res) => {
  ...
  const exists = await db.prepare('SELECT id FROM favorites WHERE user_id = ? AND listing_id = ?').get(req.user.id, listingId);
  if (exists) {
    await db.prepare('DELETE FROM favorites WHERE id = ?').run(exists.id);
    return res.json({ favorited: false });
  }
  await db.prepare('INSERT INTO favorites (user_id, listing_id) VALUES (?, ?)').run(req.user.id, listingId);
  res.json({
    favorited: true,
    item: await mapListing(await getListingById(listingId), { favoriteIds: new Set([listingId]) }),
  });
});
