...
router.post('/', requireAuth, async (req, res) => {
  ...
  const expires = type === 'bump' ? null : (await db.prepare("SELECT datetime('now', '+7 days') AS d").get()).d;
  ...
  await db.prepare('INSERT INTO payments (user_id, promotion_id, amount, method) VALUES (?, ?, ?, ?)').run(
    req.user.id,
  ...
  if (type === 'bump') {
    await db.prepare("UPDATE listings SET bumped_at = datetime('now') WHERE id = ?").run(listing.id);
  }
  if (type === 'vip') {
    await db.prepare('UPDATE listings SET is_vip = 1 WHERE id = ?').run(listing.id);
  }
  if (type === 'top') {
    await db.prepare('UPDATE listings SET is_top = 1 WHERE id = ?').run(listing.id);
  }
  ...
  res.json({ ok: true, item: await mapListing(await getListingById(listing.id)) });
});
