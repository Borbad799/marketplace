...
function conversationPayload(row, userId) {
...
router.get('/', requireAuth, (req, res) => {
  ...
  res.json({ items: rows.map((r) => conversationPayload(r, req.user.id)) });
});
...
router.post('/', requireAuth, (req, res) => {
  ...
  res.json({ item: conversationPayload(conv, req.user.id) });
});
...
router.get('/:id', requireAuth, (req, res) => {
  ...
  db.prepare('UPDATE messages SET is_read = 1 WHERE conversation_id = ? AND sender_id != ?').run(conv.id, req.user.id);
  const messages = db
    .prepare('SELECT * FROM messages WHERE conversation_id = ? ORDER BY id ASC')
    .all(conv.id)
    .map(mapMessage);
  res.json({ item: conversationPayload(conv, req.user.id), messages });
});
...
router.post('/:id/messages', requireAuth, (req, res) => {
  ...
  db.prepare("UPDATE conversations SET last_message = ?, last_message_at = datetime('now') WHERE id = ?").run(preview, conv.id);
  const message = mapMessage(db.prepare('SELECT * FROM messages WHERE id = ?').get(info.lastInsertRowid));
