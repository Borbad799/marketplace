...
async function saveDetails(listingId, type, body) {
  if (type === 'real_estate') {
    await db
      .prepare(
...
async function saveMedia(listingId, photos = [], videoUrl) {
  await db.prepare('DELETE FROM listing_media WHERE listing_id = ?').run(listingId);
  for (const [i, url] of (photos || []).entries()) {
    if (url) await db.prepare('INSERT INTO listing_media (listing_id, url, type, sort_order) VALUES (?, ?, ?, ?)').run(listingId, url, 'image', i);
  }
  if (videoUrl) {
    await db.prepare('INSERT INTO listing_media (listing_id, url, type, sort_order) VALUES (?, ?, ?, ?)').run(listingId, videoUrl, 'video', 99);
  }
}
...
router.get('/:id', optionalAuth, async (req, res) => {
  const row = await getListingById(Number(req.params.id));
  ...
  if (!isOwner) {
    await db.prepare('UPDATE listings SET views = views + 1 WHERE id = ?').run(row.id);
    row.views += 1;
  }
  const item = await mapListing(row, { favoriteIds: await favoriteIdsFor(req.user?.id) });
...
router.post('/', requireAuth, createRules, async (req, res) => {
  if (fail(req, res)) return;
  const b = req.body;
  const city = b.cityId ? await db.prepare('SELECT * FROM cities WHERE id = ?').get(Number(b.cityId)) : null;
  const info = await db
    .prepare(
  ...
  await saveDetails(id, b.type, b);
  await saveMedia(id, b.photos, b.videoUrl);
  ...
  res.status(201).json({ item: await mapListing(await getListingById(id), { favoriteIds: new Set() }) });
});

router.put('/:id', requireAuth, async (req, res) => {
  const row = await db.prepare('SELECT * FROM listings WHERE id = ?').get(Number(req.params.id));
  if (!row) return res.status(404).json({ error: 'Объявление не найдено' });
  ...
  await saveDetails(row.id, row.type, { ...row, ...b });
  if (b.photos) await saveMedia(row.id, b.photos, b.videoUrl ?? row.video_url);
  res.json({ item: await mapListing(await getListingById(row.id), { favoriteIds: await favoriteIdsFor(req.user.id) }) });
});
...
router.post('/:id/status', requireAuth, async (req, res) => {
  ...
  await db.prepare("UPDATE listings SET status = ?, updated_at = datetime('now') WHERE id = ?").run(next, row.id);
...
router.delete('/:id', requireAuth, async (req, res) => {
  ...
  const isAdmin = req.user.role === 'admin' || req.user.role === 'moderator';
  if (row.user_id !== req.user.id && !isAdmin) return res.status(403).json({ error: 'Недостаточно прав' });
  await db.prepare('DELETE FROM listings WHERE id = ?').run(row.id);
  res.json({ ok: true });
});
