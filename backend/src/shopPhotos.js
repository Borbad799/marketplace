...
export function poolFor(kind, category) {
  return CLOTHING[`${category || ''}:${kind || ''}`] || FALLBACK
}

function poolForItem(item) {
  if (item.type === 'clothing' || item.clothing) {
...
export function photosForListings(items) {
...
export function photoForRow(row, used) {
